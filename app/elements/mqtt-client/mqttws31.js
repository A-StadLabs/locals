...
Paho.MQTT = function (global) {
  ...
  */
  ...
  WireMessage.prototype.encode = function () {
    ...
    case MESSAGE_TYPE.CONNECT:
      ...
      break;  // Subscribe, Unsubscribe can both contain topic strings
    ...
    var mbi = encodeMBI(remLength);  // Convert the length to MQTT MBI format
                                     // Convert the length to MQTT MBI format
    // Convert the length to MQTT MBI format
    // Convert the length to MQTT MBI format
    var pos = mbi.length + 1;  // Offset of start of variable header
                               // Offset of start of variable header
    // Offset of start of variable header
    // Offset of start of variable header
    var buffer = new ArrayBuffer(remLength + pos);
    var byteStream = new Uint8Array(buffer);  // view it as a sequence of bytes
                                              //Write the fixed header into the buffer
                                              // view it as a sequence of bytes
                                              //Write the fixed header into the buffer
    // view it as a sequence of bytes
    //Write the fixed header into the buffer
    // view it as a sequence of bytes
    ...
    if (this.type == MESSAGE_TYPE.PUBLISH)
      pos = writeString(this.payloadMessage.destinationName, destinationNameLength, byteStream, pos);  // If this is a CONNECT then the variable header contains the protocol name/version, flags and keepalive time
    else // If this is a CONNECT then the variable header contains the protocol name/version, flags and keepalive time
    // If this is a CONNECT then the variable header contains the protocol name/version, flags and keepalive time
    if (this.type == MESSAGE_TYPE.CONNECT) {
      switch (this.mqttVersion) {
      case 3:
    ...
    case MESSAGE_TYPE.PUBLISH:
      ...
      break;  //    	    case MESSAGE_TYPE.PUBREC:	
              //    	    case MESSAGE_TYPE.PUBREL:	
  ...
  */
  ...
  // Messaging Client private instance members.
  // Messaging Client private instance members.
  ClientImpl.prototype.socket;  /* true once we have received an acknowledgement to a CONNECT packet. */
                                /* true once we have received an acknowledgement to a CONNECT packet. */
  /* true once we have received an acknowledgement to a CONNECT packet. */
  /* true once we have received an acknowledgement to a CONNECT packet. */
  ClientImpl.prototype.connected = false;  /* The largest message identifier allowed, may not be larger than 2**16 but 
  ...
  */
  ...
  ClientImpl.prototype._connectTimeout;  /* The sendPinger monitors how long we allow before we send data to prove to the server that we are alive. */
                                         /* The sendPinger monitors how long we allow before we send data to prove to the server that we are alive. */
  /* The sendPinger monitors how long we allow before we send data to prove to the server that we are alive. */
  /* The sendPinger monitors how long we allow before we send data to prove to the server that we are alive. */
  ClientImpl.prototype.sendPinger = null;  /* The receivePinger monitors how long we allow before we require evidence that the server is alive. */
  ...
  ClientImpl.prototype.store = function (prefix, wireMessage) {
    ...
    case MESSAGE_TYPE.PUBLISH:
      if (wireMessage.pubRecReceived)
        storedMessage.pubRecReceived = true;  // Convert the payload to a hex string.
      ...
      if (wireMessage.payloadMessage.retained)
        storedMessage.payloadMessage.retained = true;  // Add a sequence number to sent messages.
  ...
  */
  ...
  ClientImpl.prototype._socket_send = function (wireMessage) {
    ...
    this.socket.send(wireMessage.encode());  /* We have proved to the server we are alive. */
  ...
  */
  ClientImpl.prototype._disconnected = function (errorCode, errorText) {
    ...
    if (this._connectTimeout)
      this._connectTimeout.cancel();  // Clear message buffers.
